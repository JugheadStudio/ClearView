    ...
    if ($stmt->errno) {
      ...
      if ($row['resetToken'] === $resetToken) {
        ...
        if ($stmt->errno) {
          ...
          $errorMessage = "Database update error: " . $stmt->error;
        } elseif ($stmt->affected_rows > 0) {
          // Password updated successfully
          $successMessage = "Your password has been successfully reset.";
        } else {
          // Failed to update the password
          $errorMessage = "Failed to reset your password. Please try again later.";
        }
      } else {
        // Invalid token
    ...
    <div class="row justify-content-center login-wrapper">
      <div class="col-4 login-container">
        <div class="login-form">
          ...
          <?php elseif (isset($_POST['newPassword'])) : ?>
          ...
          <?php else : ?>
            <form method="POST" action="resetPassword.php?token=<?php echo $resetToken; ?>">
              ...
              <input type="password" id="newPassword" name="newPassword" class="form-control mb-3" required>
